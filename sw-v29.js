// =========================================================
// SERVICE WORKER: cool-dudes-lessons-cache-v29
// Pre-caches ALL pages on first visit for full offline access
// =========================================================

const CACHE_NAME = 'cool-dudes-lessons-cache-v29';
...
  console.log('[SW v29] Installing and pre-caching ALL pages...');
  
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(async (cache) => {
        // First, cache essential assets
        console.log('[SW v29] Caching essential assets...');
        await Promise.all(
          essentialAssets.map(url => 
            cache.add(url).catch(err => 
              console.warn('[SW v29] Failed to cache:', url, err)
            )
          )
        );
        
        // Then, cache all HTML pages
        console.log('[SW v29] Pre-caching all lesson pages...');
        let successCount = 0;
        let failCount = 0;
        
...
              
              successCount++;
              console.log(`[SW v29] ✓ Cached: ${page} (${successCount}/${htmlPages.length})`);
            }
          } catch (err) {
            failCount++;
            console.warn(`[SW v29] ✗ Failed: ${page}`, err);
          }
        }
        
        console.log(`[SW v29] Pre-caching complete: ${successCount} success, ${failCount} failed`);
      })
      .then(() => {
        console.log('[SW v29] Installation complete, taking control...');
...
          if (cached) {
            console.log('[SW v29] Serving from cache:', tryPath);
            
...
        try {
          console.log('[SW v29] Fetching from network:', path);
...
          return networkResponse;
        } catch (error) {
          console.log('[SW v29] Network failed, no cache available');
...
      );
    }).then(() => {
      console.log('[SW v29] Claiming clients');
