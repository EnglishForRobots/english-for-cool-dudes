const CACHE_NAME = 'cool-dudes-lessons-cache-v75'; // Increment version
...
self.addEventListener('install', (event) => {
  console.log('[SW v75] Installing... (Core Assets Only)');
        ...
        return Promise.allSettled(
          essentialAssets.map(asset => 
            cache.add(asset).catch(err => {
              console.warn(`[SW v75] Failed to cache ${asset}:`, err.message);
            })
          )
        );
      })
      .then(() => {
        console.log('[SW v75] Install complete. Skip waiting...');
...
self.addEventListener('message', (event) => {
  if (event.data && event.data.type === 'SKIP_WAITING') {
    console.log('[SW v75] Received SKIP_WAITING, activating now...');
    self.skipWaiting();
  }
});
    ...
    event.respondWith(
      (async () => {
        try {
          ...
          if (networkResponse && networkResponse.ok) {
            ...
            caches.open(CACHE_NAME).then(cache => {
              cache.put(event.request, responseToCache).catch(err => {
                console.warn('[SW v75] Cache put failed:', err);
              });
            });
          }
          ...
          console.log('[SW v75] Offline mode. Checking cache for:', requestURL.pathname);
          ...
          if (cachedResponse) {
            console.log('[SW v75] Serving from cache:', requestURL.pathname);
        ...
        const fetchPromise = fetch(event.request).then((networkResponse) => {
          ...
          if (networkResponse && networkResponse.ok) {
            cache.put(event.request, networkResponse.clone()).catch(err => {
              console.warn('[SW v75] Background cache failed:', err);
            });
          }
          return networkResponse;
        }).catch(() => {
...
self.addEventListener('activate', (event) => {
  console.log('[SW v75] Activating & cleaning old caches...');
      ...
      return Promise.all(
        cacheNames.map((cacheName) => {
          if (!cacheWhitelist.includes(cacheName)) {
            console.log('[SW v75] Deleting old cache:', cacheName);
            return caches.delete(cacheName);
          }
        })
      );
    }).then(() => {
      console.log('[SW v75] Claiming clients...');
