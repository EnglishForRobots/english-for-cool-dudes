// =========================================================
// SERVICE WORKER: cool-dudes-lessons-cache-v61
// Pre-caches ALL pages on first visit for full offline access
// =========================================================

const CACHE_NAME = 'cool-dudes-lessons-cache-v61'; // ← INCREMENT THIS EVERY UPDATE!
...
self.addEventListener('install', (event) => {
  console.log('[SW v61] Installing and pre-caching ALL pages...');
  
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(async (cache) => {
        // First, cache essential assets
        console.log('[SW v61] Caching essential assets...');
        await Promise.all(
          essentialAssets.map(url => 
            cache.add(url).catch(err => 
              console.warn('[SW v61] Failed to cache:', url, err)
            )
          )
        );
        
        // Then, cache all HTML pages
        console.log('[SW v61] Pre-caching all lesson pages...');
        let successCount = 0;
        let failCount = 0;
  ...
  headers: { 'Cache-Control': 'no-cache, no-store, must-revalidate' }
            ...
            if (response.ok) {
              ...
              successCount++;
              console.log(`[SW v61] ✓ Cached: ${page} (${successCount}/${htmlPages.length})`);
            }
          } catch (err) {
            failCount++;
            console.warn(`[SW v61] ✗ Failed: ${page}`, err);
          }
        }
        
        console.log(`[SW v61] Pre-caching complete: ${successCount} success, ${failCount} failed`);
      })
      .then(() => {
        console.log('[SW v61] Installation complete, taking control immediately...');
...
self.addEventListener('message', (event) => {
  if (event.data && event.data.type === 'SKIP_WAITING') {
    console.log('[SW v61] Received SKIP_WAITING message');
    self.skipWaiting();
  }
});
  ...
  if (isNavigation) {
    event.respondWith(
      (async () => {
        ...
        try {
          console.log('[SW v61] Fetching fresh HTML from network:', requestURL.pathname);
          ...
          console.log('[SW v61] Network failed, trying cache for:', requestURL.pathname);
          ...
          for (const tryPath of uniquePaths) {
            ...
            if (cached) {
              console.log('[SW v61] Serving from cache:', tryPath);
              return cached;
            }
          }
          
          // No cache available
          console.log('[SW v61] No cache available for:', path);
          return new Response(
            `<!DOCTYPE html>
            <html lang="en">
            <head>
      ...
      caches.match(event.request, { ignoreSearch: true })
        .then((response) => {
          ...
          return fetch(event.request).then((networkResponse) => {
            ...
            console.warn('[SW v61] Failed to fetch asset:', event.request.url);
      ...
      return Promise.all(
        cacheNames.map((cacheName) => {
          if (!cacheWhitelist.includes(cacheName)) {
            console.log('[SW v61] Deleting old cache:', cacheName);
            return caches.delete(cacheName);
          }
        })
      );
    }).then(() => {
      console.log('[SW v61] Claiming all clients immediately');
